...
public class StudentController {
    ...
    //CREATE
    @Operation(summary = "Creates a new student", method = "POST")
    @PostMapping
    ...
    //READ BY ID
    @Operation(summary = "Gets student by id", method = "GET")
    @GetMapping("/{id}")
    ...
    //UPDATE
    @Operation(summary = "Updates a student", method = "UPDATE")
    @PutMapping("/{id}")
    public ResponseEntity<StudentResponseDTO> updateStudent(@PathVariable UUID id, @Valid @RequestBody StudentRequestDTO StudentUpdateDTO) {
        return ResponseEntity.status(HttpStatus.OK).body(service.updateStudent(id, StudentUpdateDTO));
    }

    //DELETE
    @Operation(summary = "Deletes a student", method = "DELETE")
    @DeleteMapping("/{id}")
